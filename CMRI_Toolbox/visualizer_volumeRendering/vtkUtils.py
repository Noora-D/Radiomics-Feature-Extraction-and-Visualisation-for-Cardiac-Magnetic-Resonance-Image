...
def setup_brain(renderer, file):
    ...
    # thresh.SetInputConnection(brain.reader.GetOutputPort())
    ...
    colorTransferFunction.AddRGBPoint(20.0, 0.168627, 0.0, 0.0)
    colorTransferFunction.AddRGBPoint(40, 0.403922, 0.145098, 0.0784314)
    colorTransferFunction.AddRGBPoint(120, 0.780392, 0.607843, 0.380392)
    colorTransferFunction.AddRGBPoint(220, 0.847059, 0.835294, 0.788235)
    colorTransferFunction.AddRGBPoint(1024, 1, 1, 1)

    brain.volumeProperty = vtk.vtkVolumeProperty()
    brain.volumeProperty.SetColor(colorTransferFunction)
    brain.volumeProperty.SetScalarOpacity(opacityTransferFunction)
    brain.volumeProperty.ShadeOff()
    brain.volumeProperty.SetInterpolationTypeToLinear()

    brain.volume = vtk.vtkVolume()
    brain.volume.SetProperty(brain.volumeProperty)

    volumeMapper = vtk.vtkGPUVolumeRayCastMapper()
    #volumeMapper = vtk.vtkProjectedTetrahedraMapper()  # 实现了经典的投影四面体法，通过硬件加速渲染，提高了渲染效率
    #volumeMapper.SetInputConnection(trifilter.GetOutputPort())
    volumeMapper.SetInputConnection(brain.reader.GetOutputPort())
    brain.volume.SetMapper(volumeMapper)
    renderer.AddVolume(brain.volume)
...
def setup_mask(renderer, file):
    ...
    n_labels = n_labels    #改成了标签的数量不是固定的10了     n_labels = n_labels if n_labels <= 10 else 10
